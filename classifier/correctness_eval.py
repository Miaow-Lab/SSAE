...
def build_model(
    ...
    resolved_inputs = inputs or config.get("inputs", "hints")
    if resolved_inputs not in {"Tr", "hints"}:
        raise ValueError("inputs must be 'Tr' or 'hints'")

    resolved_input_dim = input_dim or config.get("n_latents") or config.get("input_dim")
    if resolved_input_dim is None:
        raise ValueError("Input dimension not found; set input_dim in correctness_eval config")
...
@torch.no_grad()
def evaluate(model, dataloader, task, inputs, device, threshold):
    total, correct = 0, 0
    for batch in dataloader:
        feats = batch["latents"] if inputs == "Tr" else batch["hints"]
...
def main():
    parser = argparse.ArgumentParser(description="Eval classifier on correctness/logicality")
    parser.add_argument("--config", type=str, default="configs/classifier.yaml")
    parser.add_argument("--set", action="append", default=[], metavar="KEY=VALUE")
    args = parser.parse_args()

    cfg = _load_section_config(args.config, "correctness_eval", args.set)

    ckpt_path = cfg.get("ckpt")
    data_file = cfg.get("data_file")
    if not ckpt_path or not data_file:
        raise ValueError("correctness_eval section requires ckpt and data_file")
    ...
    dataset = LatentsLabelDataset(data_file)
    ...
    acc, total = evaluate(model, dataloader, task, inputs, device, threshold)
